/**
 * TODO: adapt to reservations
 */
angular.module('portainer.app')
.controller('ReservationsController', ['$q', '$scope', '$state', 'ReservationService', 'TeamService', 'TeamMembershipService', 'ModalService', 'Notifications', 'Authentication', 'SettingsService',
function ($q, $scope, $state, ReservationService, TeamService, TeamMembershipService, ModalService, Notifications, Authentication, SettingsService) {
  $scope.state = {
    userCreationError: '',
    validUsername: false,
  ...
  $scope.addUser = function() {
    ...
    ReservationService.createUser(username, password, role, teamIds)
  ...
  function deleteSelectedUsers(selectedItems) {
    ...
    angular.forEach(selectedItems, function (user) {
      ReservationService.deleteUser(user.Id)
      .then(function success() {
  ...
  function initView() {
    ...
    $q.all({
      users: ReservationService.users(true),
      teams: isAdmin ? TeamService.teams() : ReservationService.userLeadingTeams(userDetails.ID),
