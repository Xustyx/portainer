...
.controller('ReserveController', ['$q', '$scope', '$state', '$transition$', 'ReservationService', 'ModalService', 'Notifications', 'SettingsService', 'Authentication',
function ($q, $scope, $state, $transition$, ReservationService, ModalService, Notifications, SettingsService, Authentication) {
  ...
  $scope.updatePermissions = function() {
    ...
    ReservationService.updateUser($scope.user.Id, undefined, role, 0)
  ...
  $scope.updatePassword = function() {
    ReservationService.updateUser($scope.user.Id, $scope.formValues.newPassword, undefined, -1)
    .then(function success() {
  ...
  function deleteUser() {
    ReservationService.deleteUser($scope.user.Id)
    .then(function success() {
  ...
  function initView() {
    ...
    $q.all({
      user: ReservationService.user($transition$.params().id),
